function VirionFinder(SeqFile,ResultFile,option1,num1,option2,num2)
load('aa_list.mat');load('aa_index.mat')
if nargin==2
    t='0';
    b='10000';
elseif nargin==3
...
while s<=seqnum
    ...
    %clear f
    VF2(aa_list,aa_index,[pwd,'/',tmp2,'file',num2str(c),'.fna'],[pwd,'/',tmp2,'out',num2str(c),'.csv'],t,tmp1);
...
if size(ResultFile,2)<4 || ~strcmp(ResultFile(end-3:end),'.csv')
    ...
    disp([ResultFile,'.csv'])
    disp('Note: The current version of VirionFinder uses "Comma-Separated Values (CSV)" as the format of the output file!!')
