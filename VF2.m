function VF2(aa_list,aa_index,SeqFile,ResultFile,t,tmp)
...
while p1<=num
    ...
    clear f
    VF(aa_list,aa_index,[pwd,'/',tmp,'file',num2str(c),'.fna'],[pwd,'/',tmp,'out',num2str(c),'.csv'],t,tmp);
...
for i=1:1:total
    ...
    elseif strcmp(predict{i,1}(1:6),'part1_')
        ...
        Group(c,1).score=sum(score.*weight);
...
for i=1:1:size(Group,1)
    ...
    %[x,y]=max(scores);
    if abs(scores-0.5)<t
        Group(i,1).Possible_source='Uncertain';
%         if x<t
...
if size(ResultFile,2)<4 || ~strcmp(ResultFile(end-3:end),'.csv')
    ...
    disp([ResultFile,'.csv'])
    disp('Note: The current version of VironFinder uses "Comma-Separated Values (CSV)" as the format of the output file!!')
